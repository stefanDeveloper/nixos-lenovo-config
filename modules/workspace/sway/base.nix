...
let
  ...
  term1 = "${pkgs.alacritty}/bin/alacritty";
...
in
{
  ...
  users.users.stefan.extraGroups = [ "sway" ];

  environment.loginShellInit = lib.mkAfter ''[[ "$(tty)" == /dev/tty1 ]] && sway'';

  home-manager.users.stefan.wayland.windowManager.sway = {
    enable = true;
    xwayland = true;
    ...
    config = rec {
      ...
      keybindings =
        ...
        in
        {
          ...
          "${modifier}+Space" =
            script "" "${pkgs.wofi}/bin/wofi --show run -f -i";
